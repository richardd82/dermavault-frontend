...
const PatientModal = ({ patient, onClose }) => {
    ...
    if (editMode) {
      ...
      if (!res.success) {
        ...
        try {
          ...
          useSearchStore.getState().setSelectedPatient(updatedPatient); // Asegura que selectedPatient se actualice
          // console.log("Stores locales actualizados");
          ...
          // onClose(); // Cierra el modal (opcional, si prefieres cerrarlo automáticamente al guardar)
          ...
          setEditMode(false); // Salimos del modo edición incluso si los stores fallaron para evitar bloqueo
        }
      }
    } else {
      setEditMode(true); // Entra en modo edición
      // Opcional: Cargar los datos más recientes del paciente desde el store
      // setFormData(useSearchStore.getState().selectedPatient || patient);
    }
  };
  ...
  // const handleToggleEdit = async () => {
  //   if (editMode) {
  //     // Llama a la API para editar al paciente
  //     const res = await editPatient(patient.id, formData);
  ...
  //       });
  //     } else {
  //       // Si la edición fue exitosa, usa la respuesta de la API (res.data)
  //       const updatedPatient = res.data;

  //       // Actualiza el paciente en el store de paginación para que se refleje en Patients.jsx
  //       updateOnePatient(updatedPatient);
  //       // Actualiza el paciente en el store de búsqueda (resultados y selectedPatient)
  //       updateOnePatientResult(updatedPatient);
  //       // También actualiza el selectedPatient directamente para asegurar que el modal tenga los datos correctos si se reabre
...
//           },
//         });
//         // Update the patient in the pagination store and search store
//         updateOnePatient(res.data);
//         updateOnePatientResult(res.data);
//         onClose();
//       }
//     } else {
//       setEditMode(true);
